      ...
      <div className="card-container">  {/* Wrap cards in a container for styling */}
        {cards.filter((card) =>
          ...
          <div key={card.id} className="card">
            ...
            <h2>{card.title}</h2>
            <p>{card.content}</p>
            <a className="cardlink font-monospace link-offset-2 link-offset-3-hover link-underline link-underline-opacity-0 link-underline-opacity-75-hover" href={card.link} target="_blank" rel="noopener noreferrer">
                  {card.linktext}
                </a>
          </div>
        ))}
